...
public:
  ...
  ~BinaryTree() { clear(); } //调用私有函数clear()完成删除

  Type getRoot() const { return root->data; }         //返回根
  Type getLeft() const { return root->left->data; }   //返回根左孩子
  Type getRight() const { return root->right->data; } //根右孩子
  ...
  // makeTree把已有的两棵树和新建立的根合并成为一棵新树
  {
    ...
    lt.root = NULL;
    rt.root = NULL;
  } //原来指向左右子树的指针设为空，避免错误P.122

  void delLeft() //删除左子树，调用clear()
  {
...
void BinaryTree<Type>::createTree(Type flag)
{
  linkQueue<Node *> que;
  Node *tmp;
  Type x, ldata, rdata; //创建树，输入flag表示空
                        //ldata和rdate分别保存输入的左右孩子值
  cout << "\n输入根结点: ";
  cin >> x;
  ...
  while (!que.isEmpty())
  {
    tmp = que.deQueue(); //tmp指向出队节点，为新节点的父亲
     ...
     if (ldata != flag)                          //若左儿子非@, 则为出队节点创建左儿子
       que.enQueue(tmp->left = new Node(ldata)); //入队新节点，为后续的创建做准备
     if (rdata != flag)                           //若右儿子非@, 则为出队节点创建右儿子
       que.enQueue(tmp->right = new Node(rdata)); //新节点入队
